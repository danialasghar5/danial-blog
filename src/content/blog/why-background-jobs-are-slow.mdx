...
<script type="application/ld+json">{`
{
  ...
  "@type": "TechArticle",
  "headline": "Why Your Rails Background Jobs Are Slow: Queue Latency vs. Execution Time",
  "description": "A “slow” background job either waited (queue latency) or ran slowly (execution time). Measure which — then a diagnostic method for fixing each, in Rails.",
  "image": "https://danialasghar.com/og/why-background-jobs-are-slow.png",
  ...
  "mainEntityOfPage": { "@type": "WebPage", "@id": "https://danialasghar.com/blog/why-background-jobs-are-slow/" },
  "keywords": ["sidekiq queue latency", "background job execution time", "rails background jobs slow", "perform.active_job", "queue latency vs execution time"],
...
<script type="application/ld+json">{`
{
  ...
  "@type": "FAQPage",
  "mainEntity": [
    { "@type": "Question", "name": "Why are my Sidekiq jobs slow?", "acceptedAnswer": { "@type": "Answer", "text": "Either they waited in the queue before a worker picked them up (queue latency) or they ran slowly once started (execution time). Measure queue latency with Sidekiq::Queue#latency; if it's high, the problem is capacity or throttling, not the job's code. If latency is low but results are slow, profile the job's perform." } },
    { "@type": "Question", "name": "What is Sidekiq queue latency?", "acceptedAnswer": { "@type": "Answer", "text": "It's the time in seconds since the oldest job in the queue was enqueued — roughly now — oldest_enqueued_at. It measures how long jobs are waiting before execution, which makes it the earliest signal of a backlog. Note it can read high for jobs re-enqueued after a deploy TERM or scheduled far in the future." } },
    { "@type": "Question", "name": "Why is my job slow to start but fast to run?", "acceptedAnswer": { "@type": "Answer", "text": "That's high queue latency with low execution time — the job waited. Common causes are too few workers for the arrival rate, a retry storm consuming capacity, connection-pool starvation, or (on Solid Queue) the polling-interval floor." } },
    { "@type": "Question", "name": "How do I measure background job execution time in Rails?", "acceptedAnswer": { "@type": "Answer", "text": "Subscribe to the perform.active_job event via ActiveSupport::Notifications. Each job run reports its duration, so you get real execution times without modifying job code or installing an APM." } },
    { "@type": "Question", "name": "Does adding more Sidekiq threads make jobs faster?", "acceptedAnswer": { "@type": "Answer", "text": "Only for I/O-bound jobs. Because of Ruby's GIL, extra threads help work that waits on the database or an API, but not CPU-bound work, where threads contend for the same core. For CPU-bound jobs, add processes instead. Also keep threads at or below your database connection pool size, or workers stall waiting for connections." } }
  ]
}
`}</script>
...
<script type="application/ld+json">{`
{
  ...
  "itemListElement": [
    ...
    { "@type": "ListItem", "position": 3, "name": "Why Your Rails Background Jobs Are Slow: Queue Latency vs. Execution Time", "item": "https://danialasghar.com/blog/why-background-jobs-are-slow/" }
  ]
}
`}</script>
