...
{/* DIAGRAM: replace this ASCII block with a designed SVG.
    Filename: idempotent-sidekiq-llm-worker-retry-flow.svg
    Alt: "Idempotent Sidekiq worker flow for LLM calls in Rails, showing where a retry hits the UUID idempotency gate" */}

```
POST /tickets ──▶ persist (PostgreSQL, UUID id) ──▶ enqueue ──▶ 202 Accepted
                                                        │
                                          ┌─────────────┘
                                          ▼
                               TriageWorker (Sidekiq)
                                          │
                          ┌───────────────┴────────────────┐
                          │  idempotency gate on the UUID   │◀── retry lands here
                          └───────────────┬────────────────┘
                                          ▼
                        call OpenAI ▶ validate response ▶ persist result
```
...
*I'm a senior backend engineer working on production Rails, async systems, and AI-integrated backends, and I'm open to senior/staff and consulting work. The best way to reach me is [on the about page](/about).*
