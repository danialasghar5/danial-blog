...
{/* JSON-LD structured data. It is valid in the document body and Google reads it from
    either head or body; if you prefer, lift these two objects into your BlogPost layout. */}
...
At-least-once delivery is not a Sidekiq flaw; it's the guarantee — [Sidekiq gives you at-least-once, not exactly-once, semantics](https://github.com/sidekiq/sidekiq/wiki/Best-Practices). A worker can be retried after a timeout, a deploy, an OOM kill, or a network blip between finishing the job and acknowledging it. For an ordinary job — recalculate a counter, resize an image — running twice is usually harmless or trivially fixable.
